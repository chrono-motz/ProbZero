...
class OthelloNet(nn.Module):
    """
    Scaled Neural Network for 6x6 Othello.
    Input:  (batch, 2, 6, 6)
    Policy: 37 logits (36 squares + 1 pass)
    ...
    def __init__(self, num_res_blocks=5, num_channels=64):
        ...
        self.r_fc1  = nn.Linear(32 * 6 * 6, num_channels)
...
class TreeDataset(Dataset):
    ...
    Binary format per sample (4x4):
      Board(32) + Policy(37) + Mask(37) + V(3) + R(3) + Valid(1) + Weight(1) = 74 floats
    """
    def __init__(self, filename):
        ...
        # Board(72) + Policy(37) + Mask(37) + V(3) + R(3) + Valid(1) + Weight(1) = 154
        floats_per_sample = 154
        ...
        # Slice columns (Updated for 6x6)
        self.boards    = arr[:,   0:72].reshape(self.len, 2, 6, 6)
        self.policies  = arr[:,  72:109]
        self.p_masks   = arr[:, 109:146]
        self.target_q  = arr[:, 146:149]
        self.target_r  = arr[:, 149:152]
        self.r_valid   = arr[:, 152]
        self.weights   = arr[:, 153]
...
def save_traced(model, filename="model_script.pt"):
    model.cpu().eval()
    example = torch.rand(1, 2, 6, 6)   # 4x4 input
