...
void print_board(const Othello& game) {
    ...
    std::cout << "\n   A B C D E F\n"; // Added E F
    for (int r = 0; r < 6; ++r) {        // Loop to 6
        std::cout << (r + 1) << " ";
        for (int c = 0; c < 6; ++c) {    // Loop to 6
            int idx = r * 6 + c;         // Multiplier to 6
            bool is_black, is_white;
            if (p == 0) {
                is_black = (buf[idx]      == 1.0f);
                is_white = (buf[36 + idx] == 1.0f); // Offset to 36
            } else {
                is_white = (buf[idx]      == 1.0f);
                is_black = (buf[36 + idx] == 1.0f); // Offset to 36
            }
...
int parse_move(std::string s) {
    if (s == "PASS" || s == "pass") return 36; // Was 16
    if (s.length() < 2) return -1;
    char col = std::toupper(s[0]);
    char row = s[1];
    if (col < 'A' || col > 'F') return -1;     // Was 'D'
    if (row < '1' || row > '6') return -1;     // Was '4'
    return (row - '1') * 6 + (col - 'A');      // Multiplier was 4
}
...
class ParallelArena {
    ...
    void run_batch_inference(std::vector<Mcts*>& batch, std::shared_ptr<Model>& model) {
        ...
        std::vector<float> input_buffer(total_leaves * 72);
        float* ptr = input_buffer.data();
        for (auto* ai : batch) {
            ai->extract_leaf_states(ptr);
            ptr += ai->get_leaf_count() * 72;
        }
...
int main(int argc, char* argv[]) {
    ...
    while (!game.is_terminal()) {
        ...
        if (ai_players[p] != nullptr) {
            ...
            if (n_legal == 1 && legal[0] == 36) {
                std::cout << "Forced Pass.\n";
                move = 36;
            } else {
                while (true) {
                    std::cout << "Enter Move (e.g., B2): ";
        ...
        for (int i = 0; i < 2; ++i) {
            if (ai_players[i] != nullptr) {
                ...
                // If the child node exists, advance the tree normally
                if (r >= 0 && ai_players[i]->nodes[r].children[move] != -1) {
                    ai_players[i]->advance_root(move);
                } else {
                    // If the node doesn't exist, hard-reset the tree to the current real board
