...
struct DataFormat {
    std::vector<float> board;       // 72 floats (2 planes of 4x4)
    std::vector<float> policy;      // 37 floats (16 squares + pass)
    std::vector<float> policy_mask; // 37 floats
    ...
    void extract_and_unify(Mcts* mcts) {
        for (int i = 0; i < mcts->node_count_idx; ++i) {
            ...
            if (entry.weight_sum == 0.0f) {
                entry.board.resize(72);
                Othello temp = node.state;
                temp.encode_state(entry.board.data());

                entry.policy_accum.resize(37, 0.0f);
                entry.policy_mask.resize(37, 0.0f);
