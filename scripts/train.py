...
class OthelloNet(nn.Module):
    def __init__(self, board_size=4, num_res_blocks=5, num_channels=64):
        ...
        self.p_fc   = nn.Linear(32 * self.board_dim, self.num_actions)

        self.v_conv = nn.Conv2d(num_channels, 32, kernel_size=1, bias=False)
        self.v_bn   = nn.BatchNorm2d(32)
        self.v_fc1  = nn.Linear(32 * self.board_dim, num_channels)
        ...
        self.r_fc1  = nn.Linear(32 * self.board_dim, num_channels)
    ...
    def forward(self, x):
        ...
        r = F.relu(self.r_bn(self.r_conv(x))).view(x.size(0), -1)
...
class TreeDataset(Dataset):
    def __init__(self, filename, board_size):
        ...
        self.num_actions = board_size * board_size + 1
        self.board_dim = board_size * board_size
        self.floats_per_sample = 2 * self.board_dim + 2 * self.num_actions + 8
        ...
        if len(raw) // 4 != self.len * self.floats_per_sample:
            print(f"Size mismatch. Recalculating...")
            self.len = len(raw) // 4 // self.floats_per_sample

        print(f"Loading {self.len} samples from {filename}...")
        arr = np.frombuffer(raw, dtype=np.float32).reshape(self.len, self.floats_per_sample)
        arr = np.nan_to_num(arr, nan=0.0)

        o1 = 2 * self.board_dim
        o2 = o1 + self.num_actions
        o3 = o2 + self.num_actions
        o4 = o3 + 3
        o5 = o4 + 3

        self.boards    = arr[:, 0:o1].reshape(self.len, 2, self.board_size, self.board_size)
        self.policies  = arr[:, o1:o2]
        self.p_masks   = arr[:, o2:o3]
        self.target_q  = arr[:, o3:o4]
        self.target_r  = arr[:, o4:o5]
        self.r_valid   = arr[:, o5]
        self.weights   = arr[:, o5+1]
...
def save_traced(model, board_size, filename="model_script.pt"):
    model.cpu().eval()
    example = torch.rand(1, 2, board_size, board_size)
