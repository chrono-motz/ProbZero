...
struct DataFormat {
    std::vector<float> board;       // 32 floats (2 planes of 4x4)
    std::vector<float> policy;      // 17 floats (16 squares + pass)
    std::vector<float> policy_mask; // 17 floats
    std::vector<float> target_q;    // 3  floats
    std::vector<float> target_r;    // 3  floats
    float reward_valid;
    float weight;
};
    ...
    void extract_and_unify(Mcts* mcts) {
        for (int i = 0; i < mcts->node_count_idx; ++i) {
            ...
            if (entry.weight_sum == 0.0f) {
                entry.board.resize(32);
                Othello temp = node.state;
                temp.encode_state(entry.board.data());

                entry.policy_accum.resize(17, 0.0f);
                entry.policy_mask.resize(17, 0.0f);
    ...
    void generate_tree_data(int flow_volume, int num_batches_per_move, std::vector<DataFormat>& dataset) {
        ...
        while (any_active) {
            ...
            for (int b = 0; b < num_batches_per_move; ++b) {
                ...
                std::vector<float> input_buffer(total_leaves * 32);
                float* ptr = input_buffer.data();

                for (auto& mcts : games) {
                    if (!mcts->is_game_over()) {
                        mcts->extract_leaf_states(ptr);
                        ptr += mcts->get_leaf_count() * 32;
                    }
                }
