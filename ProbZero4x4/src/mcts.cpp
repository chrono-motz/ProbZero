...
int Mcts::get_move(bool deterministic) {
    ...
    if (root.num_legal < 0 || root.num_legal > 17) {
    ...
    if (deterministic) {
        ...
        float epsilon = std::min(0.50f, static_cast<float>(this->iteration) / 200.0f * 0.50f);
...
void Mcts::extract_leaf_states(float* buffer){
    ...
    for (int i = 0; i < batch_size; ++i) {
        ...
        nodes[leaf_idx].state.encode_state(buffer + (i * 32));
    }
}
        ...
        if (node.state.is_terminal()) {
            ...
            if (node.state.check_win(node.state.current_player())){
             ...
             for(int m=0; m < node.num_legal; ++m) {
                ...
                float logit = p_data_base[i*17 + move];
                if (std::isnan(logit)) logit = -1e9f;
                if (logit > max_logit) max_logit = logit;
             }
             
             float sum_p = 0.0f;
             std::array<float, 17> exp_p;
...
void Mcts::final_update(int node_idx) {
    ...
    std::array<int, 17> active_children;
    int n_active = 0;
...
void Mcts::winner_prob(int num_children, const std::array<int, 17>& children, std::array<float, 17>& weights, float temp_scale) {
    weights.fill(0.0f);
    if (num_children == 0) return;
    std::array<float, 17> p0, p1, p2, lt1, lt2;
...
void Mcts::process_batch(const std::vector<int>& leaves) {
    ...
    std::vector<float> input_buffer(batch_size * 32);
    for (int i = 0; i < batch_size; ++i) {
        int leaf_idx = leaves[i];
        nodes[leaf_idx].state.encode_state(&input_buffer[i * 32]);
    }
...
void Mcts::forward_recursive(int node_idx, int volume, std::vector<int>& leaves_threaded){
    ...
    // Sanity check on num_legal
    if (node->num_legal < 0 || node->num_legal > 17) {
...
void Mcts::forward_update(int node_idx) {
    ...
    std::array<int, 17> active_child_idx;
    std::array<int, 17> active_move_idx;
    int n_active = 0;
    ...
    std::array<float, 17> scaled_probs;
...
void Mcts::print_debug_root(int limit_moves) {
    ...
    std::array<float, 17> unscaled_weights;
    ...
    std::array<float, 17> scaled_weights;
