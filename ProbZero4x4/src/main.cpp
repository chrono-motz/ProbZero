...
//   - Policy:        17 floats (16 squares + pass)
//   - Mask:          17 floats
//   - Value Target:   3 floats
//   - Reward Target:  3 floats
//   - Reward Valid:   1 float
//   - Weight:         1 float
//   Total: 32+17+17+3+3+1+1 = 74 floats
void save_dataset_binary(const std::vector<DataFormat>& dataset, const std::string& filename) {
    ...
    for (const auto& sample : dataset) {
        out.write(reinterpret_cast<const char*>(sample.board.data()),       32 * sizeof(float));
        out.write(reinterpret_cast<const char*>(sample.policy.data()),      17 * sizeof(float));
        out.write(reinterpret_cast<const char*>(sample.policy_mask.data()), 17 * sizeof(float));
